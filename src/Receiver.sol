...
contract BasicReceiver {
    ...
    mapping(bytes32 => uint256) public prevTime;
    ...
    bytes32 public constant SUBMIT_TYPEHASH = keccak256("Submit(bytes32 tag,uint256 sec,uint256 ttl,bytes32 val)");
    ...
    // EIP712 digest
    function digest(bytes32 tag, uint256 sec, uint256 ttl, bytes32 val) public view returns (bytes32) {
        ...
        bytes32 sighash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(
                        "\x19\x01", DOMAIN_SEPARATOR, keccak256(abi.encode(SUBMIT_TYPEHASH, tag, sec, ttl, val))
                    )
                )
            )
        );
    ...
    function submit(bytes32 tag, uint256 sec, uint256 ttl, bytes32 val, uint8 v, bytes32 r, bytes32 s) public {
